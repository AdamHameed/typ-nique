...
LABEL org.opencontainers.image.title="typ-nique-web"
LABEL org.opencontainers.image.description="Typ-Nique Next.js web service"
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
  CMD node -e "const port=process.env.PORT||3000; fetch(`http://127.0.0.1:${port}/api/health`, { cache: 'no-store' }).then((response)=>process.exit(response.ok?0:1)).catch(()=>process.exit(1))"
